...
ACCESS_TOKEN = "ADD Faceook Access Token Here, get it from https://developers.facebook.com/tools/explorer/"
...
FB_BASE = "https://graph.facebook.com/v2.1/"
...
def get_friend_service_ids():
    query = "me/friends?limit=50000&offset=0&access_token=%s" % ACCESS_TOKEN
...
def get_all_photos(service_id):
    query = "%s/photos?type=tagged&limit=25000&access_token=%s" % (service_id, ACCESS_TOKEN)
    ...
    for photo in all_photos:
        ...
        for tag in tag_list:
            if tag.get("name") == name:
                ...
                if 100 not in (x, y):
                    get_face_in_photo(photo_url, service_id, picture_count, name, x, y) 
                    picture_count += 1
 
 
def get_profile_picture_album_id(service_id):
    query = "%s/albums?limit=20000&access_token=%s" % (service_id, ACCESS_TOKEN)
...
def get_profile_photos(service_id, name):
    ...
    if album_id is None:
        return
    query = "%s/photos?limit=20000&access_token=%s" % (album_id, ACCESS_TOKEN)
...
if __name__ == "__main__":
    ...
    for service_id in service_ids:
        ...
        if name is None:
            continue
        get_profile_photos(service_id, name)  
        get_tagged_photos(service_id, name)  
